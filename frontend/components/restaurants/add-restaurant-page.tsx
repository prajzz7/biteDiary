...
function DishDraftCard({
  dish,
  onRemove,
}: {
  dish: DishDraftFormValues;
  onRemove: () => void;
}) {
  ...
  return (
    <article className="min-w-0 overflow-hidden rounded-card border border-border bg-bg shadow-card">
      {previewUrl ? (
            ...
            <div className="mt-2 min-w-0 rounded-control border border-border bg-surface-sunken p-3 transition focus-within:border-accent focus-within:bg-surface focus-within:ring-4 focus-within:ring-accent-soft">
              <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_74px] items-center gap-3">
                ...
                <input
                  ...
                  type="number"
                  aria-describedby={
                    errors.rating ? "dish-rating-error" : undefined
                  }
                  aria-invalid={Boolean(errors.rating)}
                  {...register("rating")}
                />
...
function DishImagesField({
  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-ink-tertiary">
            Add multiple photos for this dish before saving it.
          </p>
        </div>
        {images.length > 0 ? (
      ...
      <label
        className="mt-2 flex min-h-[112px] cursor-pointer flex-col items-center justify-center gap-2 rounded-[16px] border border-dashed border-accent/35 bg-surface-sunken px-4 text-center transition hover:border-accent/70 focus-within:border-accent focus-within:ring-4 focus-within:ring-accent-soft"
        htmlFor="dishImages"
      >
      ...
      {images.length > 0 ? (
        <div className="mt-3 grid grid-cols-3 gap-2">
          {images.map((image, index) => (
            <div
              className="relative aspect-square min-w-0 overflow-hidden rounded-[14px] border border-border bg-bg"
